...
{
		Form3->loadingIndicator->Enabled=false;
		Form3->loadingIndicator->Visible=false;
			//enable home button
		Form3->homeImageButton5->Enabled=true;
...
void __fastcall TForm3::submitButtonClick(TObject *Sender)
{
	 ...
	 Form3->errorImage->Visible=false;
	 Form3->useArrowsImage->Visible=false;
		//check to see if string contains only digits and one decimal
	for(int j=0;j<Form3->displayGrid->RowCount;++j)
	{
		int numDecimals=0;
			//check for blank input
		if(Form3->displayGrid->Cells[1][j].IsEmpty())
		{
			Form3->errorImage->Visible=true;
			return;
		}
		//put each entry into a string that will be type checked
		string inputString=AnsiString(Form3->displayGrid->Cells[1][j]).c_str();
		for (int i = 0; i < inputString.length(); i++)
		{
			//check to see if the character is 0-9 or a decimal
			if(inputString[i]<48 || inputString[i]>57)
	...
	if (msgboxID == IDYES)
	{
	Form3->loadingIndicator->Enabled=true;
	Form3->loadingIndicator->Visible=true;
